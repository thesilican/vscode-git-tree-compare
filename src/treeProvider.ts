...
class ChangeBaseCommitItem implements QuickPickItem {
  get label(): string {
    return "$(git-commit) Custom commit";
  }
  get description(): string {
    return "";
  }
...
class ChangeRepositoryItem implements QuickPickItem {
  ...
  get label(): string {
    return path.basename(this.repositoryRoot);
  }
  get description(): string {
...
export class GitTreeCompareProvider
  implements TreeDataProvider<Element>, Disposable
{
  // Events
  ...
  async init(treeView: TreeView<Element>) {
    ...
    this.disposables.push(
      workspace.onDidChangeConfiguration(this.handleConfigChange, this)
    );
    this.disposables.push(
    ...
    this.disposables.push(
      this.gitApi.onDidOpenRepository(this.handleRepositoryOpened, this)
    );
    for (const repository of this.gitApi.repositories) {
      this.disposables.push(
        repository.ui.onDidChange(() =>
          this.handleRepositoryUiChange(repository)
        )
      );
    }
    ...
    const isRelevantChange = (uri: Uri) => {
      ...
      // git ref change
      if (
        /\/\.git\/refs\//.test(uri.path) &&
        !/\/\.git\/refs\/remotes\/.+\/actions/.test(uri.path)
      ) {
        return true;
      }
      // git index change
    ...
    const onRelevantWorkspaceChange = filterEvent(
      onWorkspaceChange,
      isRelevantChange
    );
    this.disposables.push(
      onRelevantWorkspaceChange(this.handleWorkspaceChange, this)
    );

    this.disposables.push(
      treeView.onDidChangeCheckboxState(this.handleChangeCheckboxState, this)
    );
  }
  ...
  async setRepository(repositoryRoot: string) {
    ...
    if (workspaceFolders.length == 0) {
      throw new Error(
        `Could not find any workspace folder for ${repositoryRoot}`
      );
    }
  ...
  async promptChangeRepository() {
    ...
    const picks = gitRepos.map((r) => new ChangeRepositoryItem(r));
  ...
  private async handleRepositoryUiChange(repository: GitAPIRepository) {
    ...
    this.log(
      `SCM repository change detected - changing repository: ${repoRoot}`
    );
  ...
  private updateTreeRootFolder() {
    const repoIsWorkspaceSubfolder = this.repoRoot.startsWith(
      this.workspaceFolder + path.sep
    );
  ...
  private readConfig() {
    ...
    this.includeFilesOutsideWorkspaceFolderRoot = config.get<boolean>(
      "includeFilesOutsideWorkspaceRoot",
      true
    );
    this.openChangesOnSelect = config.get<boolean>("openChanges", true);
    this.autoChangeRepository = config.get<boolean>(
      "autoChangeRepository",
      false
    );
  ...
  private async getStoredBaseRef(): Promise<string | undefined> {
    ...
    if (baseRef) {
      if (
        (await this.isRefExisting(baseRef)) ||
        (await this.isCommitExisting(baseRef))
      ) {
  ...
  getTreeItem(element: Element): TreeItem {
    ...
    if (this.showCheckboxes) {
      if (element instanceof FileElement || element instanceof FolderElement) {
        checkboxState =
          this.checkboxStates.get(element.dstAbsPath) ??
          TreeItemCheckboxState.Unchecked;
      }
    }
    return toTreeItem(
      element,
      this.openChangesOnSelect,
      this.iconsMinimal,
      this.showCollapsed,
      this.viewAsList,
      checkboxState,
    ...
    if (!element) {
      ...
      if (!this.filesInsideTreeRoot) {
        try {
          ...
          this.log(
            "Ignoring updateDiff() error during initial getChildren()",
            e
          );
          return [new RepoElement(path.basename(this.repoRoot))];
        }
      }
      const hasFiles =
        this.filesInsideTreeRoot.size > 0 ||
        (this.includeFilesOutsideWorkspaceFolderRoot &&
          this.filesOutsideTreeRoot.size > 0);

      return [
        new RepoElement(path.basename(this.repoRoot)),
        new RefElement(this.repoRoot, this.baseRef, hasFiles),
      ];
      ...
      return this.getFileSystemEntries(
        element.dstAbsPath,
        element.useFilesOutsideTreeRoot
      );
    }
  ...
  private async updateRefs(baseRef?: string): Promise<void> {
    ...
    try {
      ...
      const headCommit =
        HEAD.commit ||
        (await getBranchCommit(this.absGitCommonDir, HEAD.name!));
      if (baseRef) {
        const exists =
          (await this.isRefExisting(baseRef)) ||
          (await this.isCommitExisting(baseRef));
      ...
      if (!baseRef) {
        if (HEAD.name) {
          ...
          const refs = await this.repository!.getRefs();
          // Compare against remote refs first
          const remoteRefNames = ["origin/main", "origin/master"];
          const remoteRef = refs.find(
            (ref) => ref.name !== undefined && remoteRefNames.includes(ref.name)
          );
          const commonRefNames = ["main", "master"];
          const commonRef = refs.find(
            (ref) =>
              ref.name !== undefined && commonRefNames.indexOf(ref.name) !== -1
          );
          if (remoteRef) {
            baseRef = remoteRef.name;
          } else if (commonRef) {
            baseRef = commonRef.name;
      ...
      if (this.headCommit !== headCommit) {
        this.log(
          `HEAD ref commit updated: ${this.headCommit} -> ${headCommit}`
        );
      }
  ...
  private async updateDiff(fireChangeEvents: boolean) {
    ...
    const diff = await diffIndex(
      this.repository!,
      this.mergeBase,
      this.refreshIndex,
      this.findRenames
    );
    const untrackedCount = diff.reduce(
      (prev, cur, _) => prev + (cur.status === "U" ? 1 : 0),
      0
    );
    ...
    for (const entry of diff) {
      ...
      const isInsideTreeRoot =
        folder === this.treeRoot || folder.startsWith(this.treeRoot + path.sep);
      const files = isInsideTreeRoot
        ? filesInsideTreeRoot
        : filesOutsideTreeRoot;
    ...
    if (fireChangeEvents) {
      const hasChanged = (folderPath: string, insideTreeRoot: boolean) => {
        const oldFiles = insideTreeRoot
          ? this.filesInsideTreeRoot
          : this.filesOutsideTreeRoot;
        const newFiles = insideTreeRoot
          ? filesInsideTreeRoot
          : filesOutsideTreeRoot;
        const oldItems = oldFiles
          .get(folderPath)!
          .map((f) => `${f.status}|${f.dstAbsPath}`);
        const newItems = newFiles
          .get(folderPath)!
          .map((f) => `${f.status}|${f.dstAbsPath}`);
        for (const { files, items } of [
          { files: oldFiles, items: oldItems },
      ...
      const treeRootChanged =
        !this.filesInsideTreeRoot ||
        !filesInsideTreeRoot.size !== !this.filesInsideTreeRoot.size;
      const mustAddOrRemoveRepoRootElement =
        !this.filesOutsideTreeRoot ||
        !filesOutsideTreeRoot.size !== !this.filesOutsideTreeRoot.size;
      if (treeRootChanged || mustAddOrRemoveRepoRootElement) {
        treeHasChanged = true;
      } else {
        for (const folder of filesInsideTreeRoot.keys()) {
          if (
            !this.filesInsideTreeRoot.has(folder) ||
        ...
        if (!treeHasChanged) {
          for (const folder of filesOutsideTreeRoot.keys()) {
            if (
              !this.filesOutsideTreeRoot.has(folder) ||
  ...
  private async isHeadChanged() {
    ...
    if (this.headName) {
      ...
      const headCommit = await getBranchCommit(
        this.absGitCommonDir,
        this.headName
      );
      if (this.headCommit !== headCommit) {
        return true;
  ...
  private async handleWorkspaceChange(uri: Uri) {
    ...
    if (!window.state.focused || !this.treeView.visible) {
      ...
      const onDidBecomeVisible = filterEvent(
        this.treeView.onDidChangeVisibility,
        (e) => e.visible
      );
      const onDidFocusWindowOrBecomeVisible = anyEvent<any>(
        onDidFocusWindow,
        onDidBecomeVisible
      );
    ...
    if (await this.isHeadChanged()) {
      ...
      try {
        ...
        // some error occured, ignore and try again next time
        this.log(
          "Ignoring updateRefs() error during handleWorkspaceChange()",
          e
        );
        return;
      }
  ...
  private async handleConfigChange() {
    ...
    this.readConfig();
    if (
      oldTreeRootIsRepo != this.treeRootIsRepo ||
  ...
  private getFileSystemEntries(
    folder: string,
    useFilesOutsideTreeRoot: boolean
  ): FileSystemElement[] {
    const entries: FileSystemElement[] = [];
    const files = useFilesOutsideTreeRoot
      ? this.filesOutsideTreeRoot
      : this.filesInsideTreeRoot;
    ...
    if (this.viewAsList) {
      ...
      for (const folder2 of files.keys()) {
        if (path.dirname(folder2) === folder) {
          ...
          entries.push(
            new FolderElement(label, compactedPath, useFilesOutsideTreeRoot)
          );
        }
      }
      entries.sort((a, b) =>
        a.label
          .split(path.sep, 1)[0]
          .localeCompare(b.label.split(path.sep, 1)[0])
      );
    } else {
      ...
      for (const folder2 of files.keys()) {
        if (path.dirname(folder2) === folder) {
          ...
          entries.push(
            new FolderElement(label, folder2, useFilesOutsideTreeRoot)
          );
        }
      }
      entries.sort((a, b) =>
        path.basename(a.dstAbsPath).localeCompare(path.basename(b.dstAbsPath))
      );
    }
  ...
  private getDiffStatus(fileEntry?: FileElement): IDiffStatus | undefined {
    ...
    const files = isInsideTreeRoot
      ? this.filesInsideTreeRoot
      : this.filesOutsideTreeRoot;
    const diffStatus = files
      .get(folder)
      ?.find((file) => file.dstAbsPath === dstAbsPath);
    return diffStatus;
  }
  ...
  async openChanges(fileEntry?: FileElement) {
    ...
    await this.doOpenChanges(
      diffStatus.srcAbsPath,
      diffStatus.dstAbsPath,
      diffStatus.status
    );
  }
  ...
  openAllChanges(
    entry: RefElement | RepoRootElement | FolderElement | undefined
  ) {
    const withinFolder =
      entry instanceof FolderElement ? entry.dstAbsPath : undefined;
  ...
  async doDiscardChanges(statuses: IDiffStatus[]) {
    ...
    for (const diffStatus of statuses) {
      ...
      if (diffStatus.status === "U") {
        ...
        actions.push(async () => {
          await this.repository!.checkout(this.mergeBase, [
            diffStatus.dstAbsPath,
          ]);
        });
    ...
    if (prompts.length === 1) {
      ...
      const answer = await window.showWarningMessage(msg, { modal: true }, btn);
      if (answer !== btn) {
        return;
      }
      actions[0]();
  ...
  *iterFiles(withinFolder: string | undefined = undefined) {
  ...
  async promptChangeBase() {
    ...
    const refs = (
      await this.repository.getRefs({ sort: "committerdate" })
    ).filter((ref) => ref.name);
    const heads = refs
      .filter((ref) => ref.type === RefType.Head)
      .map((ref) => new ChangeBaseRefItem(ref));
    const tags = refs
      .filter((ref) => ref.type === RefType.Tag)
      .map((ref) => new ChangeBaseTagItem(ref));
    const remoteHeads = refs
      .filter((ref) => ref.type === RefType.RemoteHead)
      .map((ref) => new ChangeBaseRemoteHeadItem(ref));
    ...
    window.withProgress(
      { location: ProgressLocation.Window, title: "Updating Tree Base" },
      async (_) => {
        try {
  ...
  async manualRefresh() {
    window.withProgress(
      { location: ProgressLocation.Window, title: "Updating Tree" },
      async (_) => {
        try {
          if (await this.isHeadChanged()) {
            // make sure merge base is updated when switching branches
  ...
  viewAsTree(v: boolean) {
    const viewAsList = !v;
    if (viewAsList === this.viewAsList) return;
    this.viewAsList = viewAsList;
    commands.executeCommand(
      "setContext",
      NAMESPACE + ".viewAsList",
      viewAsList
    );
    this.log("Refreshing tree");
    this._onDidChangeTreeData.fire();
  ...
  async searchChanges() {
    ...
    const relativePaths = uris.map((uri) =>
      path.relative(this.repoRoot, uri.fsPath)
    );
...
function toTreeItem(
  element: Element,
  openChangesOnSelect: boolean,
  iconsMinimal: boolean,
  ...
  if (element instanceof FileElement) {
    ...
    item.tooltip = `${element.refName} (${path.basename(
    ...
    return item;
  } else if (element instanceof RepoElement) {
